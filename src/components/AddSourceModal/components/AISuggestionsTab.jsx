...
export function AISuggestionsTab({
  ...
  const addSelectedSources = async () => {
    ...
    const selectedSourcesArray = Array.from(selectedSuggestions).map(
      (i) => persistedSuggestions[i]
    );
    try {
      ...
      if (actualSources.length < expectedCount) {
        alert(
          `${
            expectedCount - actualSources.length
          } source(s) were not added because they already exist`
        );
      }
      clearSelections();
      onCancel();
    ...
    >
      <div className="space-y-4">
        ...
        <SuggestionsList
          ...
          onAddSelected={addSelectedSources}
        />
      </div>
    </div>
  );
