...
export function Sources({ sources = [], onSourcesUpdate, isUpdating = false }) {
  ...
  const modalProps = {
    newSource, setNewSource, onAdd: addSource, onCancel: () => setShowAddModal(false),
    isAdding: addingSource, sources, onSourcesUpdate, persistedTopic, setPersistedTopic,
    persistedSuggestions, setPersistedSuggestions
  };

  if (!sources || sources.length === 0) {
    return (
      <>
        <EmptySourcesState onAddClick={() => setShowAddModal(true)} isUpdating={isUpdating} />
  ...
  return (
    <div className="space-y-4">
      <AddSourceButton onClick={() => setShowAddModal(true)} isUpdating={isUpdating} />
      {sources.map((source, index) => (
        <SourceCard key={index} source={source} index={index} onRemove={removeSource} isUpdating={isUpdating} />
      ))}
      {showAddModal && <AddSourceModal {...modalProps} />}
    </div>
  );
